...
export function ProductForm({ productId, initialData }: ProductFormProps) {
  ...
  const {
    ...
    defaultValues: {
      ...
      priceUsd: toNum(initialData?.priceUsd) ?? (undefined as unknown as number),
  ...
  // Number input register helper
  const numReg = (name: keyof ProductInput, opts?: { required?: boolean }) => ({
    ...register(name, {
      ...(opts?.required ? { required: true } : {}),
      setValueAs: (v: string) => {
        if (v === "" || v === undefined || v === null) return opts?.required ? undefined : null
        const n = parseFloat(v)
        return isNaN(n) ? (opts?.required ? undefined : null) : n
      },
    }),
  })
